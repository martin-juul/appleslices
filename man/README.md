...
The aslice build does this for every `*.1.md` here and installs the results into the prefix's `share/man/man1/`, which `aslice shellenv` puts on `MANPATH`. No other tooling is involved, deliberately — the source stays readable as plain Markdown on the web and in editors.
