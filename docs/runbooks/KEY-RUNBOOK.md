...
For an isolated security release, first bind the incident to the exact production baseline and the `maintenance/<incident-id>/develop` → `/beta` → `/master` branch mapping in [ORCHARD-POLICY](../ORCHARD-POLICY.md#maintenance-releases). Verify the closed maintenance promotion record, identical artifact inventories, owner authorization, staging evidence, and every ordinary gate. Recheck the production baseline under the activation fence; any concurrent change requires reconciliation and a new candidate/version. Retain forward-integration status until the fix reaches ordinary development. Drill wrong-branch, missing-gate, and concurrent-baseline refusals before enabling this path.
...
| September 2026 | Specify maintenance promotion with unchanged gates and artifacts, exact production-baseline fencing, and forward integration tracking. |
