...
Each gap: what Homebrew has, why it matters *specifically for aslice*, and the concrete proposal.
...
**aslice today:** §13.4 says CI "builds the package in the sandbox on both flavors" (now three). Under-specified at exactly the points where the ABI model creates new obligations.
