...
- **Status:** Design draft, v1.5 — September 2026
...
ABI scanner with DWARF diffing; SBOM + `audit`; SLSA provenance; `aslice-toolchain` v2 (LLD-first linking, ccache integration); extended orchard to ~2,000 packages; popular-variant prebuilds chosen from community requests (§9.4); reproducible builds for core; vendor-binary packages (`type = "binary"`, payload extraction, signer pinning) and the Cask importer; `aslice repo build/publish` for third-party repositories; **runtime version management** — the shim layer, `use`/`pin`/`default`, riding tools, and version-bound extension slices with the farm's runtime-epoch build axis (§12.9).
