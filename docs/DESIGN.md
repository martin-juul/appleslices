...
- **Status:** Design draft, v1.18 — September 2026
...
- Vendor binary packages: accepted into extended with accurate OS-support tags and a pinned signer when the artifact is signed — unsigned artifacts are extended-only, `signer` omitted, announced at every install (§12.2, §12.4); into core only if signed, hosted by the farm (`redistribute = true`), and payload-only by construction. A vendor package whose scripts turn out to be required is removed, not accommodated.
