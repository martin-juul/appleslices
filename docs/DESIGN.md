...
**The formula importer** of §13.3 ships in this group as `aslice orchard port --from-homebrew <formula>` — named *port*, because §13.3 is honest that only the simple majority of Ruby formulae translate mechanically; the rest are ports, and the command says so. (`aslice import --from-brewfile`, §12.13, remains the user-side verb: migrating a machine, not authoring a formula.)
