...
- **Status:** Design draft, v1.22 — September 2026
...
| Role | Hardware | Notes |
|---|---|---|
| Primary `v1`/`v2` builder and tester | 2013 Mac Pro (trashcan) | Also coordinator, compatible OS-test VMs, and restricted signing VM |
...
The signing host is the Mac Pro's restricted VM, with signing credentials and token access unavailable to build guests; root custody stays offline (§10.2). A compromise of the shared physical host or hypervisor can defeat this isolation. Losing the Mac Pro also loses the coordinator and signing service until recovery (BUILD-INFRA §11).
