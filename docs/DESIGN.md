...
- **Vendor binaries are the same format, minus the build.** `type = "binary"` packages describe a `.pkg`/`.dmg` artifact with per-OS tags, a pinned signer, and a declarative payload map — no `build.star`, and vendor scripts only as declared, approved grafts (§12.4, §12.15; PACKAGE-FORMAT §3.11).
...
Install of a `.slice` is: verify signature → verify payload hashes → extract into store path → ABI-check against the packages that will link to it → register in SQLite → link into profile. **No code from the package executes at any point** — unless the package declares grafts and the user has approved them, in which case the grafts run sandboxed to their behavior manifest between extraction and linking (§12.15). A repackaged vendor binary produces the same `.slice` shape — its manifest's provenance section records the vendor artifact hash and signer instead of a build recipe.
...
The signing host is an offline release Pi; a separate offline Pi holds the root (§10.2). The Mac Pro's restricted publisher VM prepares bundles and publishes verified signatures; it holds only the timestamp key among repository signing keys. Build guests have no publication credentials. Shared-host compromise can corrupt evidence and publication and expose the timestamp key, but does not directly expose offline keys. Losing the Mac Pro stops preparation and publication until recovery (BUILD-INFRA §11).
...
- **Authoring → publishing.** `aslice repo build` compiles an orchard (git formulae) — or a bare manifest directory — into a repository tree; `aslice repo sign` applies the keys; `aslice repo publish` pushes to the configured transport. The official pipeline prepares a portable bundle online, signs it on the offline release Pi, and verifies and atomically publishes the returned batch (KEY-RUNBOOK §2.1). These remain the same authoring commands; offline handling adds no client signature format.
...
- **Metadata:** the index is wrapped in [The Update Framework](https://theupdateframework.io/) — a 1-of-1 root on an offline Pi, distinct targets and snapshot keys on a separate offline release Pi, and only the timestamp key online. Keys are Ed25519; private offline keys and their backups are encrypted. Root metadata lasts one year, targets/snapshot 90 days, and timestamps 48 hours with daily refresh. Alert 30 days before offline metadata expires; timestamp refresh cannot extend snapshot or targets expiry (KEY-RUNBOOK §1.1). This gives rollback, freeze, and mix-and-match attack protection — the failure modes that plain "signed packages" miss.
...
- **One scoped exception: `aslice-system`.** Declared system-software packages (kexts, SIP-disabled development tools — §12.7) require privileged steps no user-space manager can perform. Those steps are executed by a single tiny auditable helper that elevates **per operation, with explicit consent, for only the declared actions** — it is not a daemon, holds no ambient authority, and every invocation is an unsuppressible logged security event (§12.5). Its scope also covers **system-domain service operations** — bootstrapping and removing root LaunchDaemons for declared `domain = "system"` services (§12.8) — and **declared `[system-patch]` file replacements** (§12.11: backup, symlink swap, restore), and **elevated graft execution** (§12.15: approved vendor scripts whose manifest declares `elevated = true`) — with the same per-operation consent and logging; user-domain agents never touch it. The steady-state rule stands: nothing else in aslice ever elevates.
...
aslice machine export [--defaults com.apple.dock,…]   # capture this machine as an aslice-machine.toml — leaves + selections; preferences on demand (§12.13)
...
**Lifecycle verbs — the `[deprecation]` table, edited by command.** Schema in PACKAGE-FORMAT §3.14, policy in ORCHARD-POLICY §8:
...
Some vendor packages genuinely cannot install payload-only: audio DSP suites, pro-video plugin frameworks, and driver bundles ship `.pkg` installers whose scripts register components with the OS, install kexts, or lay down files the payload map cannot express. ProTools-class software is the norm on this platform, not the exception, and refusing these packages wholesale would hand the users aslice most wants to serve back to manual installs — unprovenanced, unmonitored, and irreversible. So installer scripts are in scope, under the project's own word and its own rules: a vendor install script is a **graft**, and a graft never runs unannounced, unapproved, or unrecorded.

- **Declared, or it doesn't run.** A graft-bearing formula declares each script in `[[binary.graft]]` (PACKAGE-FORMAT §3.11): the script's path inside the package, its sha256, when it runs (before or after payload extraction), and a **behavior manifest** — the complete list of what the script is expected to do: the paths it may write outside the payload map (`writes`), the kexts it may install (`kexts`), the launchd jobs it may register (`daemons`), whether it may touch the network (`network = true`, default false), and whether it needs elevation (`elevated = true` — performed through `aslice-system`, §10.4). A script discovered in an artifact but not declared is a lint error; a declared script whose hash drifts is a hard fail; behavior beyond the manifest is blocked by construction, not merely reported.
- **Approved like pnpm, persisted two ways.** Installing a graft-bearing package prompts before anything executes: the package, each graft, and its behavior manifest are printed, and the user is asked whether to allow this package to run its scripts. Interactively, an approval is recorded in the state DB — bound to the package, version, and graft hashes, so a changed script asks again. Declaratively, the machine file carries a `[grafts]` allow-list (`allow = ["protools-hd"]`, SETUP §2.8), so an approved set rebuilds a machine without prompting. Non-interactive installs without a matching approval are refused (exit 2) unless `--accept-grafts` is passed — the same contract shape as `--accept-system-changes`, one severity level down. Recorded approvals are reviewable and revocable with `aslice graft approvals` / `aslice graft revoke`. There is no "allow everything, always" switch: the unit of approval is one package's grafts at one version.
- **Monitored while it runs.** The graft executes under a Seatbelt profile derived from its behavior manifest: writes outside the declared `writes` set are denied by the sandbox, network access is denied unless declared, and the process tree is watched for the classics — child shells escaping the profile, `kextload` beyond the declared kexts, persistence attempts outside the declared daemons. A deviation aborts the install and rolls the generation back; the abort, the deviation, and everything the script did up to it are logged unsuppressibly (§12.5).
- **Captured, so rollback stays true.** Every write the graft makes inside its declared set is recorded against the new generation in the state DB: created files are listed, overwritten files are backed up first (the §12.11 backup mechanism, generalized), and registered kexts and daemons are noted. `aslice rollback` and `aslice uninstall` reverse the graft's footprint together with the generation — G7's rollback claim extends to grafts, or the package doesn't ship.
- **Rehearsed, verified, and signed by the farm.** For core and extended, the farm rehearses every graft-bearing install in a per-OS VM across the artifact's declared range: the install runs under instrumentation, the observed writes, loads, and network contact are diffed against the declared behavior manifest, and only on an exact match does the signing host sign the **verified manifest** into the index (REPOSITORIES §3). A script that does more than its manifest says fails rehearsal and never ships. Verified repositories sign their own manifests under their vouched key — the same statement, one trust level down.
- **Unsigned manifests carry the big fat warning.** A graft-bearing package whose manifest is unsigned — third-party orchards, or an official package whose rehearsal has not run — installs only after an unmissable warning: the word UNSIGNED in the largest type the terminal gives us, the repository's provenance line, the full declared behavior, and a per-decision prompt that is never persisted. The user's risk, taken with open eyes — but taken deliberately.

Grafts compose with, never replace, the declarative categories: a kext the payload map can express still goes through `[system]` (§12.7), and a package whose scripts turn out to be unnecessary is rewritten payload-only at review. The default remains §10.1's: no code at install. The graft is the declared, approved, monitored, reversible exception — and it exists because the alternative is the user running the same script by hand, as root, with none of the above.
...
- Single-owner start: the project owner makes launch decisions and operates offline signing. Add independent maintainers and agree multi-party custody when participation warrants it; neither a founding quorum nor hardware-token purchases block launch (KEY-RUNBOOK §5).
...
| Signing-key compromise | Medium | Offline root and release Pis, encrypted backups, pre-launch recovery drills, explicit root-compromise rebootstrap, transparency log for detection |
