...
The owner's merge into the configured protected orchard release branch is the final human approval during single-owner launch, including new core slices. The existing `aslice repo build / sign / publish` stages exchange a release candidate automatically. This is a design specification: candidate delivery, signer service, publication coordination, and acceptance drills remain implementation work. Client signature formats remain unchanged. Candidate authorization, gate receipts, and signing journals are internal control-plane records, not new fields in the closed TUF or index schemas.
