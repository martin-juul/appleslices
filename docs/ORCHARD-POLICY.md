...
- **Status:** Policy v0.4 — September 2026 (v0.2: kernel extensions and SIP-disabled development software move from hard rejection to the restricted, warned, trust-gated system-software category — §2, new §13; mechanism in DESIGN v1.2 §12.7. v0.3: service-package acceptance — root-domain daemons meet §13's bar, user agents meet the normal tier bar — §13; mechanism in DESIGN v1.3 §12.8. v0.4: the system-file hard rejection becomes the declared `[system-patch]` category — §2, §13; mechanism in DESIGN v1.7 §12.11 — and the trust-store slices (`ca-certificates`, `apple-roots`) are recorded as ordinary pinned data-only core packages — §9; mechanism in DESIGN v1.7 §12.10)
- **Companion to:** [DESIGN.md](DESIGN.md) v1.7, [PACKAGE-FORMAT.md](PACKAGE-FORMAT.md) v0.5, [BUILD-INFRA.md](BUILD-INFRA.md) v0.1, [REPOSITORIES.md](REPOSITORIES.md) v0.5, [HOMEBREW-REVIEW.md](HOMEBREW-REVIEW.md) v0.8
...
**Precedence.** The specifications define *mechanism* — what fields exist, what the solver does, what CI can check. This file defines *policy* — what maintainers accept, require, and refuse. Where the two appear to conflict, the conflict is a bug: file an issue against whichever document is wrong. Schema fields referenced here that are not yet in PACKAGE-FORMAT.md v0.5 (`[deprecation]`, `[livecheck]`, `link`/`link_reason`, `notes`, `ctx.replace`, `[system-patch]`) are the pending amendments listed in HOMEBREW-REVIEW.md §8; they are normative policy from this version onward and land in PACKAGE-FORMAT v0.6. (`[system]` landed in PACKAGE-FORMAT v0.4 §3.12, alongside the `[service]` table of §3.8; `[runtime]`/`[extension]`/`[ride]` landed in v0.5 §3.13.)
...
- **Trust gating is policy, and it is the strictest in the system.** Only **official and local** repositories may serve `[system-patch]` packages (the `system-patch` capability, REPOSITORIES.md §3 amendment); **verified repositories may not, third-party never.** A verified community repository is vetted to distribute software; rewriting the OS is a judgment the project keeps to itself and the machine's owner. (DESIGN open question #10 may revisit verified; until it resolves, the gate stands.)
