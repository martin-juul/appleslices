...
6. **Post-merge signing** — slice and metadata signing happen only after merge, on the offline release Pi; the publisher verifies the returned batch before atomic publication (KEY-RUNBOOK §2.1). No PR, however trusted, touches keys.
...
- **Orchard review grants no signing authority.** The project owner operates the offline release Pi after merge; publication requires its signed batch (KEY-RUNBOOK §2.1). A compromised maintainer account can open PRs; it cannot sign slices.
...
- **The index publishes in manual release batches:** eligible merges queue for the next offline signing session (KEY-RUNBOOK §2.1). The publisher atomically activates each verified batch; daily timestamp refresh maintains freshness for its valid approved snapshot and does not publish queued changes.
