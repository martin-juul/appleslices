...
- **Trust gating is policy, and it is the strictest in the system.** Only **official and local** repositories may serve `[system-patch]` packages (the `system-patch` capability, REPOSITORIES.md §3); **verified repositories may not, third-party never.** A verified community repository is vetted to distribute software; rewriting the OS is a judgment the project keeps to itself and the machine's owner. (DESIGN open question #10 may revisit verified; until it resolves, the gate stands.)
