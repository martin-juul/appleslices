...
- **Declare the real floor, skip the heroics** (DESIGN §4.1): a package that *could* be patched down to 10.11 but isn't worth the effort declares its real floor (10.12, 10.14, whatever it is) and moves on. Users plan around accurate tags; they file angry issues around optimistic ones. Patch heroics to lower a floor are welcome — as patches (§7), with the floor lowered only when CI proves it.
...
- **A dead upstream is not a dead package.** Every source the farm fetches is vendored into the repository (DESIGN §9.6), so an upstream that vanishes or reshuffles its downloads breaks nothing already built. When a formula's canonical URL dies, the maintainer notes the death in the formula (a comment naming the date and the mirror situation) and livecheck adapts or the package moves toward §8.
...
- **Licensing is documented.** `redistribute = true` requires a license basis (permission, license terms, or clearly redistributable freeware) noted in the formula; `redistribute = false` yields a pointer formula. A vendor pulling or mutating an artifact produces a hash failure by design — never a silent install of different bits.
