...
6. **Post-merge signing** — slice and metadata signing happen automatically only after owner-approved merge and all required gates, on the dedicated networked release Pi; the publisher verifies the returned candidate before atomic publication ([KEY-RUNBOOK §2.1](KEY-RUNBOOK.md#automatic-orchard-to-client-publication)). No PR, however trusted, touches keys.
