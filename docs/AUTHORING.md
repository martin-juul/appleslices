...
**The gate, before the farm.** `aslice orchard ci ffmpeg` runs chapter 10's six gates locally: lint, a sandboxed build per declared flavor at `min_os`, the smoke test, the ABI diff against the published index, and graft rehearsal on your OS. It is the same harness the farm runs, so a green local run makes the farm run a formality — except the cross-OS smoke and graft-rehearsal tiers, which only the farm's VMs can do, and the output says so. Run it bare on a PR branch to scope to the changed formulae; `--all` is the whole orchard and warns you what that costs.
...
*History: v0.12 (September 2026) — prose rewrite of the iteration guidance, formula introduction, and repository publishing explanation; no content changes.*
