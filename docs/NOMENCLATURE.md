...
**signing host** — the offline release Pi: verifies release bundles and signs slices, targets, and snapshots with distinct keys; never runs package builds. A separate offline Pi holds root authority. *(BUILD-INFRA §5, §7; KEY-RUNBOOK §1.)*

**publisher** — the restricted VM on the owned Mac Pro that checks gates, prepares bundles, verifies returned offline signatures, and publishes atomically. Holds the timestamp key and publication credentials, but no root or release private keys. *(BUILD-INFRA §5, §9.)*
...
**quarantine** — the untrusted staging area every agent result lands in; nothing is ever served from it, and publication requires the six gates, offline release signing, and publisher verification. *(BUILD-INFRA §7.1.)*
