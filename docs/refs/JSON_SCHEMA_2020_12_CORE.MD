...
Navigation: [Account](#captured-material) | [Source context](#decision-relevant-context) | [Full original](#full-captured-source) | [Catalog](README.MD)

## Source identity

- Original source: https://www.ietf.org/archive/id/draft-bhutton-json-schema-01.txt
- Author/publisher: JSON Schema specification authors
- Source version/date: 2020-12 dialect; draft header dated 2022-06-10 (earlier wrapper recorded 2022-06-16)
- Retrieved: 2026-09-26 (UTC date; web and HTTP capture)
- Capture type: complete upstream text with attribution and license
- Rights: Original notices and redistribution terms retained in the full source.
- Used by:
  - [schematics/README — The three schema languages](../../schematics/README.md#the-three-schema-languages): JSON schematics use the 2020-12 dialect and local schema resources.
  - [schematics/README — Authority and precedence](../../schematics/README.md#authority-and-precedence): Structural schemas do not replace prose semantics.

## Captured material

The full fixed draft below preserves the schema language used by the JSON schematics.

## Decision-relevant context

Source locator: Retained draft: §4 definitions, §8 core vocabulary, §9 loading schemas, §10 applicators, §11 unevaluated locations, §12 output.

The core draft defines the instance model, boolean/object schemas, schema resources, and reference resolution. `$schema` selects a dialect; `$id` establishes resource identity and a base URI. `$ref` applies the referenced schema and does not by itself prohibit other keywords alongside it. Resource identity is not an instruction to fetch untrusted network content: §9 separates loading and associating resources from their evaluation.

Applicators such as `allOf`, `anyOf`, `oneOf`, and `if`/`then`/`else` combine subschema results. `properties` evaluates named members; it does not require their presence. `additionalProperties` applies to names not covered in that schema object, whereas `unevaluatedProperties` depends on successful evaluation annotations across applicable subschemas. Confusing these mechanisms can accidentally reopen or over-close composed objects.

For offline use, load the project's schema resources into a local registry before validating fixtures. A missing resource is a resolution problem, not a successful validation. The retained output section supplies validity/error-location concepts; the validation companion supplies type, numeric, string, and presence assertions.

## Applicability and limits

This fixes the 2020-12 dialect; it does not silently adopt future drafts.

## Project interpretation

The local registry and fixed dialect make structural checks reproducible without retrieving schemas. Cryptographic authorization, payload inspection, and relationships among separately validated files remain semantic gates. A schema passing its own meta-schema only establishes schema shape.

## Omissions and evidence gaps

The complete fixed draft is retained without omissions. Referenced external standards and later errata are not automatically included. The header says June 10, 2022; the previous wrapper said published June 16. That discrepancy is recorded rather than altering the source.
...
Exact source bytes: [JSON_SCHEMA_2020_12_CORE.source.txt](JSON_SCHEMA_2020_12_CORE.source.txt). Retrieved URL: https://www.ietf.org/archive/id/draft-bhutton-json-schema-01.txt. SHA-256: `03834e2a60caff313476df9913dac2ccf9a34aad569df570f8ab80db7bd6d24a`.


Review: 2026-09-26 UTC. Original source re-read against the cited local sections; added locators, conditions, and explicit gaps. The previously retained source bytes were re-fetched and matched exactly; embedded originals and notices are unchanged.
...
Wright, et al.          Expires 12 December 2022                [Page 1]

...
Wright, et al.          Expires 12 December 2022                [Page 2]

Internet-Draft                 JSON Schema                     June 2022


       7.6.1.  Assertions and Instance Primitive Types . . . . . . .  18
     7.7.  Annotations . . . . . . . . . . . . . . . . . . . . . . .  19
       7.7.1.  Collecting Annotations  . . . . . . . . . . . . . . .  19
     7.8.  Reserved Locations  . . . . . . . . . . . . . . . . . . .  23
     7.9.  Loading Instance Data . . . . . . . . . . . . . . . . . .  23
   8.  The JSON Schema Core Vocabulary . . . . . . . . . . . . . . .  23
     8.1.  Meta-Schemas and Vocabularies . . . . . . . . . . . . . .  24
       8.1.1.  The "$schema" Keyword . . . . . . . . . . . . . . . .  25
     ...
     8.2.  Base URI, Anchors, and Dereferencing  . . . . . . . . . .  27
       8.2.1.  The "$id" Keyword . . . . . . . . . . . . . . . . . .  28
       8.2.2.  Defining location-independent identifiers . . . . . .  29
       8.2.3.  Schema References . . . . . . . . . . . . . . . . . .  30
       8.2.4.  Schema Re-Use With "$defs"  . . . . . . . . . . . . .  31
     8.3.  Comments With "$comment"  . . . . . . . . . . . . . . . .  32
   9.  Loading and Processing Schemas  . . . . . . . . . . . . . . .  32
     9.1.  Loading a Schema  . . . . . . . . . . . . . . . . . . . .  32
       9.1.1.  Initial Base URI  . . . . . . . . . . . . . . . . . .  33
       9.1.2.  Loading a referenced schema . . . . . . . . . . . . .  33
       9.1.3.  Detecting a Meta-Schema . . . . . . . . . . . . . . .  34
     9.2.  Dereferencing . . . . . . . . . . . . . . . . . . . . . .  34
       9.2.1.  JSON Pointer fragments and embedded schema
               resources . . . . . . . . . . . . . . . . . . . . . .  35
     9.3.  Compound Documents  . . . . . . . . . . . . . . . . . . .  37
       9.3.1.  Bundling  . . . . . . . . . . . . . . . . . . . . . .  38
       9.3.2.  Differing and Default Dialects  . . . . . . . . . . .  38
       9.3.3.  Validating  . . . . . . . . . . . . . . . . . . . . .  39
     9.4.  Caveats . . . . . . . . . . . . . . . . . . . . . . . . .  39
       9.4.1.  Guarding Against Infinite Recursion . . . . . . . . .  39
       9.4.2.  References to Possible Non-Schemas  . . . . . . . . .  39
     9.5.  Associating Instances and Schemas . . . . . . . . . . . .  40
       9.5.1.  Usage for Hypermedia  . . . . . . . . . . . . . . . .  40
   10. A Vocabulary for Applying Subschemas  . . . . . . . . . . . .  41
     10.1.  Keyword Independence . . . . . . . . . . . . . . . . . .  41
     10.2.  Keywords for Applying Subschemas in Place  . . . . . . .  42
       10.2.1.  Keywords for Applying Subschemas With Logic  . . . .  42
       10.2.2.  Keywords for Applying Subschemas Conditionally . . .  43
     10.3.  Keywords for Applying Subschemas to Child Instances  . .  44
       10.3.1.  Keywords for Applying Subschemas to Arrays . . . . .  45
       10.3.2.  Keywords for Applying Subschemas to Objects  . . . .  46
   11. A Vocabulary for Unevaluated Locations  . . . . . . . . . . .  48
     11.1.  Keyword Independence . . . . . . . . . . . . . . . . . .  49
     11.2.  unevaluatedItems . . . . . . . . . . . . . . . . . . . .  49
     11.3.  unevaluatedProperties  . . . . . . . . . . . . . . . . .  50
   12. Output Formatting . . . . . . . . . . . . . . . . . . . . . .  51
     12.1.  Format . . . . . . . . . . . . . . . . . . . . . . . . .  51
     12.2.  Output Formats . . . . . . . . . . . . . . . . . . . . .  51



Wright, et al.          Expires 12 December 2022                [Page 3]

Internet-Draft                 JSON Schema                     June 2022


     12.3.  Minimum Information  . . . . . . . . . . . . . . . . . .  52
       12.3.1.  Keyword Relative Location  . . . . . . . . . . . . .  52
       12.3.2.  Keyword Absolute Location  . . . . . . . . . . . . .  52
       12.3.3.  Instance Location  . . . . . . . . . . . . . . . . .  53
       12.3.4.  Error or Annotation  . . . . . . . . . . . . . . . .  53
       12.3.5.  Nested Results . . . . . . . . . . . . . . . . . . .  53
     12.4.  Output Structure . . . . . . . . . . . . . . . . . . . .  53
       12.4.1.  Flag . . . . . . . . . . . . . . . . . . . . . . . .  55
       12.4.2.  Basic  . . . . . . . . . . . . . . . . . . . . . . .  55
       12.4.3.  Detailed . . . . . . . . . . . . . . . . . . . . . .  56
       12.4.4.  Verbose  . . . . . . . . . . . . . . . . . . . . . .  58
       12.4.5.  Output validation schemas  . . . . . . . . . . . . .  60
   13. Security Considerations . . . . . . . . . . . . . . . . . . .  60
   14. IANA Considerations . . . . . . . . . . . . . . . . . . . . .  61
     14.1.  application/schema+json  . . . . . . . . . . . . . . . .  61
     14.2.  application/schema-instance+json . . . . . . . . . . . .  61
   15. References  . . . . . . . . . . . . . . . . . . . . . . . . .  62
     15.1.  Normative References . . . . . . . . . . . . . . . . . .  62
     15.2.  Informative References . . . . . . . . . . . . . . . . .  62
   Appendix A.  Schema identification examples . . . . . . . . . . .  63
   ...
   Appendix E.  References and generative use cases  . . . . . . . .  72
   Appendix F.  Acknowledgments  . . . . . . . . . . . . . . . . . .  73
...
Wright, et al.          Expires 12 December 2022                [Page 4]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022                [Page 5]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022                [Page 6]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022                [Page 7]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022                [Page 8]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022                [Page 9]

Internet-Draft                 JSON Schema                     June 2022
...
4.3.5.  Root Schema and Subschemas and Resources
   ...
   identified by an absolute URI [RFC3986].  Schema resources MAY also
   be identified by URIs, including URIs with fragments, if the
   resulting secondary resource (as defined by section 3.5 of RFC 3986
   [RFC3986]) is identical to the primary resource.  This can occur with
   the empty fragment, or when one schema resource is embedded in
   another.  Any such URIs with fragments are considered to be non-
   canonical.
...
Wright, et al.          Expires 12 December 2022               [Page 10]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 11]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 12]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 13]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 14]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 15]

Internet-Draft                 JSON Schema                     June 2022
...
7.4.  Identifiers
   ...
   resolved in references (Section 8.2.3), or both.  The Core vocabulary
   defined in this document defines several identifying keywords, most
   notably "$id".
...
Wright, et al.          Expires 12 December 2022               [Page 16]

Internet-Draft                 JSON Schema                     June 2022
...
7.5.  Applicators
   ...
   Annotation (Section 7.7) results are preserved along with the
...
Wright, et al.          Expires 12 December 2022               [Page 17]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 18]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 19]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 20]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 21]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 22]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 23]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 24]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 25]

Internet-Draft                 JSON Schema                     June 2022
   ...
   unrecognized keywords that are defined in vocabularies from those
...
Wright, et al.          Expires 12 December 2022               [Page 26]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 27]

Internet-Draft                 JSON Schema                     June 2022
...
8.2.1.  The "$id" Keyword
   ...
   a fragment), or to a URI with an empty fragment.

   The empty fragment form is NOT RECOMMENDED and is retained only for
   backwards compatibility, and because the application/schema+json
   media type defines that a URI with an empty fragment identifies the
   same resource as the same URI with the fragment removed.  However,
   since this equivalence is not part of the RFC 3986 normalization
   process [RFC3986], implementers and schema authors cannot rely on
   generic URI libraries understanding it.

   Therefore, "$id" MUST NOT contain a non-empty fragment, and SHOULD
   NOT contain an empty fragment.  The absolute-URI form MUST be
   considered the canonical URI, regardless of the presence or absence
   of an empty fragment.
   // An empty fragment is currently allowed because older meta-schemas
   // have an empty fragment in their $id (or previously, id).  A future
   // draft may outright forbid even empty fragments in "$id".

   The absolute-URI also serves as the base URI for relative URI-
   references in keywords within the schema resource, in accordance with
   RFC 3986 section 5.1.1 [RFC3986] regarding base URIs embedded in
   content.
...
Wright, et al.          Expires 12 December 2022               [Page 28]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 29]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 31]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 32]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 33]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 34]

Internet-Draft                 JSON Schema                     June 2022
   ...
   // known?  Are there circumstances in which automatic network
   // dereferencing is allowed?  A same origin policy?  A user-
   // configurable option?  In the case of an evolving API described by
   // Hyper-Schema, it is expected that new schemas will be added to the
   // system dynamically, so placing an absolute requirement of pre-
...
Wright, et al.          Expires 12 December 2022               [Page 35]

Internet-Draft                 JSON Schema                     June 2022
   ...
   with schema references (Section 8.2.3), or is structured as a single
...
Wright, et al.          Expires 12 December 2022               [Page 36]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 37]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 38]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 39]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 40]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 41]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 42]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 43]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 44]

Internet-Draft                 JSON Schema                     June 2022
...
10.3.1.2.  items
   ...
   against this keyword's subschema.  This annotation affects the
   behavior of "unevaluatedItems" in the Unevaluated vocabulary.
...
Wright, et al.          Expires 12 December 2022               [Page 45]

Internet-Draft                 JSON Schema                     June 2022
...
10.3.1.3.  contains
   ...
   elements is valid against the given schema, except when "minContains"
   is present and has a value of 0, in which case an array instance MUST
   be considered valid against the "contains" keyword, even if none of
...
Wright, et al.          Expires 12 December 2022               [Page 46]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 47]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 48]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 49]

Internet-Draft                 JSON Schema                     June 2022
   ...
   boolean true, analogous to the behavior of "items".  This annotation
...
Wright, et al.          Expires 12 December 2022               [Page 50]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 51]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 52]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 53]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 54]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 55]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 56]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 57]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 58]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 59]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 60]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 61]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 62]

Internet-Draft                 JSON Schema                     June 2022
   ...
   [W3C.WD-fragid-best-practices-20121025]
              ...
              fragid-best-practices-20121025, 25 October 2012,
   ...
   [json-schema-validation]
              ...
              json-schema-validation-01, June 2022,
...
Wright, et al.          Expires 12 December 2022               [Page 63]

Internet-Draft                 JSON Schema                     June 2022
   ...
   # (document root)  canonical (and base) URI  https://example.com/
         root.json

                      canonical resource URI plus pointer fragment  http
         s://example.com/root.json#

   #/$defs/A  base URI  https://example.com/root.json

              canonical resource URI plus plain fragment  https://exampl
         e.com/root.json#foo

              canonical resource URI plus pointer fragment  https://exam
         ple.com/root.json#/$defs/A
   ...
   #/$defs/B  canonical (and base) URI  https://example.com/other.json
         ...
         ple.com/other.json#
              ...
              base URI of enclosing (root.json) resource plus
      fragment  https://example.com/root.json#/$defs/B




Wright, et al.          Expires 12 December 2022               [Page 64]

Internet-Draft                 JSON Schema                     June 2022
   ...
   #/$defs/B/$defs/X  base URI  https://example.com/other.json
                      ...
                      canonical resource URI plus plain fragment  https:
         //example.com/other.json#bar

                      canonical resource URI plus pointer fragment  http
         s://example.com/other.json#/$defs/X

                      base URI of enclosing (root.json) resource plus
      fragment  https://example.com/root.json#/$defs/B/$defs/X

   #/$defs/B/$defs/Y  canonical (and base) URI  https://example.com/t/
         inner.json

                      canonical URI plus plain fragment  https://example
         .com/t/inner.json#bar

                      canonical URI plus pointer fragment  https://examp
         le.com/t/inner.json#

                      base URI of enclosing (other.json) resource
      plus fragment  https://example.com/other.json#/$defs/Y

                      base URI of enclosing (root.json) resource plus
      fragment  https://example.com/root.json#/$defs/B/$defs/Y

   #/$defs/C  canonical (and base) URI  urn:uuid:ee564b8a-
         7a87-4125-8c96-e9f123d6766f

              canonical URI plus pointer fragment  urn:uuid:ee564b8a-7a8
         7-4125-8c96-e9f123d6766f#

              base URI of enclosing (root.json) resource plus
      fragment  https://example.com/root.json#/$defs/C
   ...
   // Pointer fragments and embedded schema resources (Section 9.2.1)
...
Wright, et al.          Expires 12 December 2022               [Page 65]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 66]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 67]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 68]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 69]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 70]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 71]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 72]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 73]

Internet-Draft                 JSON Schema                     June 2022
...
Appendix G.  ChangeLog
   ...
   draft-bhutton-json-schema-01
      *  Improve and clarify the "type", "contains",
         "unevaluatedProperties", and "unevaluatedItems" keyword
         explanations
   ...
   draft-bhutton-json-schema-00
      ...
      *  "contains" annotation behavior, and "contains" and
         "unevaluatedItems" interactions now specified
...
Wright, et al.          Expires 12 December 2022               [Page 74]

Internet-Draft                 JSON Schema                     June 2022


      *  Moved "unevaluatedItems" and "unevaluatedProperties" from core
...
Wright, et al.          Expires 12 December 2022               [Page 75]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 76]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 77]

Internet-Draft                 JSON Schema                     June 2022
...
Wright, et al.          Expires 12 December 2022               [Page 78]
