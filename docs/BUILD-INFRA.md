...
| Existing release Pi | Offline targets, snapshot, and slice signing; no package builds |
...
The **signing host** is a dedicated offline release Pi holding distinct targets, snapshot, and slice-signing keys. A separate offline root Pi holds the 1-of-1 root. Both use encrypted software keys and offline backups; KEY-RUNBOOK governs custody and recovery. The restricted VM on the Mac Pro is the **publisher**: it checks gates, prepares portable bundles, verifies returned signatures, and publishes, with network access for staging, definitions, and publication. It holds repository credentials and the timestamp key only; build guests receive neither. Shared-host compromise can corrupt evidence and publication and expose the timestamp key (§7.2), but does not directly expose offline keys. The Pis are existing signing devices, not package builders; validate their OS and signing-tool support before launch.
...
agent → staging (untrusted)
      → publisher: quarantine gates → repo build → portable bundle
      → removable media → offline release Pi: verify → review → repo sign
      → removable media → publisher: re-verify → repo publish (atomic)
      → transparency-log append (snapshot hash, public)
      → dashboard update
```

KEY-RUNBOOK §2.1 specifies bundle contents, retained signing state, untrusted-media handling, and rejection/retry behavior. The root Pi is not used for routine batches. The publisher refreshes timestamps daily only for the approved snapshot; offline metadata renewal and key rotation remain separate procedures (KEY-RUNBOOK §1.1, §3).
...
| Release Pi down | No new releases or offline renewals; daily timestamps may continue for valid approved metadata, but expired snapshot/targets metadata blocks updates; restore and drill before signing |
...
| Mac Pro down | Primary builds, coordinator, guests, preparation, and publication stop; offline keys remain on Pis. Recover coordinator and publisher state; replace a lost/suspect timestamp key through a root update. Updates fail closed when metadata expires |
...
- **Phase 1:** coordinator + agents + leases; Actions adapter; VM matrix bring-up; PR gates for the core orchard; staging → quarantine → offline signing → publisher pipeline including the §7.5 malware-signature gate, bootstrapped per its genesis protocol, with the `clamav` core package the gate runs on.
