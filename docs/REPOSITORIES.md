...
Official authoring uses a portable release bundle: `repo build` runs on the publisher after gates, `repo sign` on the offline release Pi, and `repo publish` on the publisher after signature and content verification. Bundle contents, approval, atomic activation, and retries are specified in KEY-RUNBOOK §2.1. Root metadata is valid for one year, targets/snapshot for 90 days, and timestamps for 48 hours with daily refresh. Offline renewals are scheduled with 30-day alerts; online refresh cannot extend offline metadata expiry. No client signature-format or command-group change is introduced.
...
| A `third-party` repo serves a graft-bearing package | Allowed, but the manifest is unsigned: the client shows the unsigned-graft warning with the full declared behavior, and approval is per decision, never persisted (§3; DESIGN §12.15) |
