...
This is the from-zero runbook: how the entire project — keys, toolchain, manager, orchard, repository, farm — is brought into existence, and how it is brought *back* into existence after a total loss. It exists because a project that can only be born once is a project that dies once. Every chicken-and-egg pair in the system has a documented path here; when a new one is introduced, it gets a row in §2 and a step in §1 **before the change lands**, the same discipline as the key runbook.

The standing rule, inherited from the toolchain genesis, the TUF ceremony, and the scanner genesis (BUILD-INFRA §7.5): **genesis events are documented exceptions with receipts, never silent gaps.**

---

## 1. The from-nothing sequence

Executed in order. Steps 1–5 need one Intel Mac running the newest Intel macOS available, Apple's Command Line Tools, this repository, and the root custodians. Nothing else — no aslice, no orchard, no farm, no network services of ours.
...
The order is load-bearing: keys before metadata, toolchain before manager, manager before orchard, orchard before repository, repository before installer, all of it before the first user.

## 2. The genesis inventory

Every "what makes the thing that makes the thing" pair in the system, its path, and where it is specced. New genesis dependencies get a row here before they merge.
...
Everything the project cannot regenerate must exist in **at least two independent locations**, one of them off GitHub, one of them offline. Losing any single item is an inconvenience; losing the set is the disaster path (§4).
...
GitHub gone, farm flooded, domain lapsed — the scenario where §1 must run from the archive alone.

1. Recover the never-lose set from its second location. Verify the ceremony archive against a root YubiKey before trusting anything else in it — the root is the anchor; everything else is verified *from* it.
...
Annually, on a clean machine, using **only** the never-lose set: run §1 end-to-end through step 9 — a wiped 10.11 VM installing from a re-standup repository. The drill is minuted like a key ceremony, and its result is binary: either the project stood up from nothing, or the gap found gets a row in §2 and a fix before anything else ships. A genesis that has never been executed is assumed not to work — the same rule the keys live by (KEY-RUNBOOK §7).
